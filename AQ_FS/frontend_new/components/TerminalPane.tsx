...
const TerminalPane: React.FC<TerminalPaneProps> = ({
  ...
  onSendMessage, onCreateSession, onSwitchSession, onDeleteSession, onDeleteMessage,
  isVisible, isCloning, cloneId, pipelineSteps,
}) => {
        ...
        <div className="pl-3 flex-1 min-w-0">
          ...
          {meta.domUpdateStatus && (
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[10px] text-gray-400 font-mono">{meta.domUpdateStatus}</span>
            </div>
          )}
  ...
  return (
    <aside className="flex-1 md:flex-none w-full md:w-[420px] flex flex-col bg-[#09090b] relative shrink-0 z-30 md:border-l border-border-dark h-full">
      {/* Terminal Header */}
      <div className="h-10 px-4 border-b border-border-dark flex items-center justify-between shrink-0 bg-[#09090b]">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-gray-300">TERMINAL</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col font-mono text-sm min-h-full">
            {currentSession.messages.length === 0 && (
              <div className="flex-1 flex flex-col items-center justify-center text-gray-700 space-y-2 opacity-50 select-none">
                ...
                <span className="text-[10px]">Start a new command sequence...</span>
              </div>
            )}
      ...
      {/* Input Area */}
      <div className="shrink-0 bg-[#09090b] border-t border-border-dark p-0">
        <div className="flex flex-col">
          {/* Toolbar */}
          <div className="flex items-center gap-3 px-3 py-1.5 border-b border-border-dark bg-[#0c0c0e]">
            <button
            ...
            >
              <span className="material-symbols-outlined text-[12px]">add</span> NEW CONTEXT
            </button>
            ...
            {isCloning && (
              <span className="flex items-center gap-1">
                ...
                <span className="text-[9px] text-gray-500 font-mono">Processing</span>
              </span>
            )}
          </div>

          {/* Text input */}
          <div className="flex items-start px-2 py-3 gap-2 bg-[#050505] relative">
            <span className="text-blue-500 pt-2 font-mono text-sm select-none">&#10095;</span>
            <textarea
              ref={textareaRef}
              className="w-full bg-transparent text-gray-200 text-sm font-mono placeholder-gray-600 p-2 focus:ring-0 border-none resize-none h-20 custom-scrollbar leading-relaxed outline-none"
              placeholder={
                cloneId
                  ? 'Ask Claude to make changes...'
                  : 'Enter a URL to clone (e.g. https://example.com)...'
              }
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              onKeyDown={handleKeyDown}
              disabled={showHistory || isCloning}
            />

            {/* Action buttons */}
            <div className="flex flex-col gap-2 pt-1 absolute right-2 top-3">
              <button
                onClick={handleSend}
                disabled={!inputValue.trim() || isCloning}
                className="p-1.5 rounded bg-white text-black hover:bg-gray-200 transition-colors shadow-sm disabled:opacity-30 disabled:cursor-not-allowed"
                title="Send"
              >
